exports.run = (client, message, args) => {
    ...
    else if(setStatus[1] === 'idie'){
        client.user.setStatus('idie')
            .then(message.channel.send("My status has been set to: "+setStatus[1] + "(idie)"))
            .catch(console.error);
    } 

    else if(setStatus[1] === 'online'){
        client.user.setStatus('online')
            .then(message.channel.send("My status has been set to: "+ setStatus[1] + "(online)"))
            .catch(console.error);
    }

    else if(setStatus[1] === 'invisible'){
        client.user.setStatus('invisible')
            .then(message.channel.send("My status has been set to: "+ setStatus[1] + "(invisible)"))
            .catch(console.error);
    }
